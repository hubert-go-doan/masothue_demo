...
RSpec.describe 'Admin::CompaniesControllers', type: :request do
  ...
  let(:represent) { create(:represent) }
  ...
  describe 'GET #search' do
    context 'with valid search query' do
      it 'returns a success response with search results' do
        create(:company, name: 'ABC')
        get search_company_admin_companies_path, params: { q: 'ABC' }
        expect(response).to have_http_status(200)
      end
    end
    ...
    context 'with invalid search query' do
      it 'returns a success response with no search results' do
        get search_company_admin_companies_path, params: { q: 'Nonexistent' }
        expect(response).to have_http_status(200)
      end
    end
  end
  ...
  describe 'POST #create' do
    context 'with valid parameters' do
      ...
      it 'returns 302 status' do
        post admin_companies_path(valid_params)
    ...
    context 'with invalid parameters' do
      ...
      it 'returns unprocessable_entity status' do
        post admin_companies_path(invalid_params)
  ...
  describe 'PUT #update' do
    context 'with valid parameters' do
      it 'updates the company and redirects to index' do
        new_name = 'Updated Name'
        put admin_company_path(company), params: { company: { name: new_name } }
        expect(response).to have_http_status(302)
      end
    end
    ...
    context 'with invalid parameters' do
      it 'returns unprocessable_entity status' do
        put admin_company_path(company), params: { company: { name: '' } }
        expect(response).to have_http_status(422)
      end
    end
  end
