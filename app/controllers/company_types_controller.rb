class CompanyTypesController < ApplicationController
  ...
  def show
    ...
    @pagy, @data = pagy_array((@company_type.companies.includes(:tax_code, :represent, :city, :district, :ward) + @company_type.people.includes(:tax_code, :city, :district, :ward)).sort_by(&:date_start).reverse)
